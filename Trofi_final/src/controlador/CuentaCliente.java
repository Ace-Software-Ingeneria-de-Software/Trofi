...
public class CuentaCliente extends HttpServlet {
	...
	private void crearCuenta(HttpServletRequest request, HttpServletResponse response) throws SQLException, ServletException, IOException {
		...
		Cliente verificador = clienteDAO.obtenerPorCorreo(correo_e);
		String mensaje = "Error: inalncanzable."; 
		if(verificador != null) {
			System.out.println("verif existe");
			mensaje = "Ya existe una cuenta con el correo "+correo_e +", por favor utiliza una cuenta de correo diferente.";
			RequestDispatcher dispatcher = request.getRequestDispatcher("/jsp/CuentaCliente/error.jsp");
			request.setAttribute("mensaje", mensaje);
			dispatcher.forward(request, response);
		}else {
			System.out.println("verif no existe");
			Cliente cliente = new Cliente(correo_e, request.getParameter("nombre"), request.getParameter("a_paterno"), request.getParameter("a_materno"), request.getParameter("contrasenia"), request.getParameter("telefono"));
			...
			mensaje = "Creacion de cuenta exitosa."; 
	...
	private void mostrarPerfil(HttpServletRequest request, HttpServletResponse response) throws SQLException, ServletException, IOException {
		if(request.getParameter("correo") != "" && request.getParameter("correo") != null ) {
			System.out.println("CC cambio Sesion");
			correo = request.getParameter("correo");
		}
		System.out.println("CC Sesion: "+ correo);
	...
	private void actualizarCuenta(HttpServletRequest request, HttpServletResponse response) throws SQLException, ServletException, IOException {
		...
		String mensaje = "Actualizaci�n de cuenta exitosa"; 
		RequestDispatcher dispatcher = request.getRequestDispatcher("/jsp/CuentaCliente/mensaje.jsp");
		request.setAttribute("correo", correo);
		request.setAttribute("mensaje", mensaje);
		dispatcher.forward(request, response);
